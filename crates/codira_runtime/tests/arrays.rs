...
fn arrays() {
    let driver = CompileAndRunTestDriver::new(
        r"
    public func main() -> [i32] { [5,4,3,2,1] }
    ",
        |builder| builder,
    )
...
fn array_of_structs() {
    let driver = CompileAndRunTestDriver::new(
        r"
    public struct Number { value: i32 };
    public struct Value { value: i64, other: i64 };

    public func main() -> [Number] { [Number { value: 2351 }, Number { value: 18571 }] }
    public func main_value() -> [Value] { [Value { value: 253, other: 1823512 }, Value { value: 123, other: 436501 }] }
    ",
        |builder| builder,
    )
    .expect("Failed to build test driver");
...
fn arrays_as_argument() {
    let driver = CompileAndRunTestDriver::new(
        r"
    public func generate() -> [i32] { [5,4,3,2,1] }
    public func add_one(array: [i32], len: usize) -> [i32] {
        let i = 0;
        loop {
            array[i] += 1;
            i += 1;
