...
/// Represents a Codira struct pointer.
#[repr(transparent)]
#[derive(Clone)]
...
impl<'s> StructRef<'s> {
    /// Creates a `StructRef` that wraps a raw Codira struct.
    ...
    /// Consumes the `StructRef`, returning a raw Codira struct.
...
impl<'s> Marshal<'s> for StructRef<'s> {
    ...
    fn marshal_into<'r>(self) -> Self::CodiraType {
        self.into_raw()
    }
    ...
    fn marshal_to_ptr(value: Self, mut ptr: NonNull<Self::CodiraType>, type_info: &Type) {
        let struct_info = type_info.as_struct().unwrap();
...
impl RootedStruct {
    /// Creates a `RootedStruct` that wraps a raw Codira struct.
