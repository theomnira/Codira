...
/// Represents a Codira array pointer.
#[repr(transparent)]
#[derive(Clone)]
...
impl<'array, T: Marshal<'array> + 'array> ArrayRef<'array, T> {
    /// Creates a `ArrayRef` that wraps a raw Codira struct.
    ...
    /// Consumes the `ArrayRef`, returning a raw Codira array.
...
impl<'a, T: Marshal<'a> + 'a> Marshal<'a> for ArrayRef<'a, T> {
    type CodiraType = RawArray;

    fn marshal_from<'runtime>(value: Self::CodiraType, runtime: &'runtime Runtime) -> Self
    where
        Self: 'a,
        'runtime: 'a,
    {
    ...
    fn marshal_to_ptr(value: Self, mut ptr: NonNull<Self::CodiraType>, _type_info: &Type) {
...
impl<T> RootedArray<T> {
    /// Creates a `RootedArray` that wraps a raw Codira struct.
