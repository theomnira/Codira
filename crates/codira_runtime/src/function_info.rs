...
use codira_memory::{type_table::TypeTable, HasStaticType, TryFromAbiError, Type};

/// A linked version of [`codira_abi::FunctionDefinition`] that has resolved all
...
/// A linked version of [`codira_abi::FunctionPrototype`] that has resolved all
/// occurrences of `TypeId` with `TypeInfo`.
#[derive(Clone)]
...
/// A linked version of [`codira_abi::FunctionSignature`] that has resolved all
/// occurrences of `TypeId` with `TypeInfo`.
#[derive(Clone)]
...
macro_rules! into_function_info_impl {
    ($(
        ...
        $(
            impl<$R: codira_memory::HasStaticType, $($T: codira_memory::HasStaticType,)*> IntoFunctionDefinition
            for extern "C" fn($($T),*) -> $R
            {
                fn into<S: Into<String>>(self, name: S) -> FunctionDefinition {
                    FunctionDefinition {
                        ...
                        prototype: FunctionPrototype {
                            ...
                            signature: FunctionSignature {
                                arg_types: vec![$(<$T as codira_memory::HasStaticType>::type_info().clone(),)*],
                                return_type: <R as codira_memory::HasStaticType>::type_info().clone(),
                            }
                        }
                    }
                }
