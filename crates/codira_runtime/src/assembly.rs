...
/// An error that occurs upon loading of a Codira library.
#[derive(Debug, thiserror::Error)]
pub enum LoadError {
    #[error("An assembly with the same name already exists")]
    AlreadyExists,
    #[error(transparent)]
    FailedToLoadSharedLibrary(#[from] codira_libloader::InitError),
    #[error("ABI version mismatch. codiralib is `{actual}` but runtime is `{expected}`")]
    MismatchedAbiVersions { expected: u32, actual: u32 },
    #[error(transparent)]
    Other(#[from] io::Error),
}

/// An error that occurs upon linking of a Codira assembly.
#[derive(Debug, thiserror::Error)]
pub enum LinkError {
...
/// An error that occurs upon linking of a Codira function prototype.
#[derive(Debug, thiserror::Error)]
...
impl Assembly {
    ...
    pub unsafe fn load(library_path: &Path, gc: Arc<GarbageCollector>) -> Result<Self, LoadError> {
        let mut library = CodiraLibrary::new(library_path)?;
