...
pub(crate) struct LanguageServerState {
    ...
    /// The vfs monitor
    pub vfs_monitor: Box<dyn codira_vfs::Monitor>,

    /// The receiver of vfs monitor messages
    pub vfs_monitor_receiver: Receiver<codira_vfs::MonitorMessage>,
    ...
    pub packages: Arc<Vec<codira_project::Package>>,
...
impl LanguageServerState {
    ...
    #[allow(clippy::unnecessary_wraps)]
    fn handle_vfs_task(&mut self, mut task: codira_vfs::MonitorMessage) -> anyhow::Result<()> {
        loop {
            match task {
                codira_vfs::MonitorMessage::Progress { total, done } => {
                ...
                codira_vfs::MonitorMessage::Loaded { files } => {
