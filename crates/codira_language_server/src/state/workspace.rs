...
use codira_hir_input::{FileId, PackageSet, SourceRoot, SourceRootId};
use codira_paths::{AbsPathBuf, RelativePath};
...
impl LanguageServerState {
    ...
    pub(crate) fn fetch_workspaces(&mut self) {
        ...
        let packages = self
            ...
            .flatten()
            .filter_map(
                |project| match codira_project::Package::from_file(project.path) {
                    Ok(package) => Some(package),
                    Err(err) => {
                        self.show_message(
                            lsp_types::MessageType::ERROR,
                            format!("codira failed to load package: {err:#}"),
                        );
                        None
                    }
                },
        ...
        if self.config.watcher == FilesWatcher::Client {
            let registration_options = lsp_types::DidChangeWatchedFilesRegistrationOptions {
                watchers: packages
                    .iter()
                    .map(|package| format!("{}/**/*.code", package.source_directory().display()))
        ...
        let entries_to_load = packages
            ...
            .map(|package| {
                ...
                codira_vfs::MonitorEntry::Directories(codira_vfs::MonitorDirectories {
        ...
        let monitor_config = codira_vfs::MonitorConfig {
