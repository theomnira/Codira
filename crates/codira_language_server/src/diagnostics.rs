...
use codira_hir::{AstDatabase, InFile, Module};
use codira_hir_input::{FileId, ModuleId, PackageId, SourceDatabase};
use codira_syntax::{Location, TextRange};
...
pub(crate) fn diagnostics(db: &AnalysisDatabase, file_id: FileId) -> Vec<Diagnostic> {
    ...
    let mut sink = codira_hir::diagnostics::DiagnosticSink::new(|d| {
