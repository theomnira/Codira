...
#[no_mangle]
pub unsafe extern "C" fn codira_struct_type_guid(ty: StructInfo, guid: *mut Guid) -> ErrorHandle {
...
/// has been deallocated by a previous call to [`codira_type_release`].
#[no_mangle]
pub unsafe extern "C" fn codira_struct_type_memory_kind(
    ty: StructInfo,
...
#[no_mangle]
pub unsafe extern "C" fn codira_fields_find_by_name(
    fields: Fields,
    name: *const c_char,
    len: usize,
    has_field: *mut bool,
    ...
    *has_field = false;
    ...
    for f in fields {
        let field_inner = codira_error_try!(f.inner());
...
/// been deallocated by a previous call to [`codira_fields_destroy`].
#[no_mangle]
pub unsafe extern "C" fn codira_fields_destroy(fields: Fields) -> ErrorHandle {
    if fields.fields.is_null() && fields.count > 0 {
...
/// has been deallocated by a previous call to [`codira_type_release`].
#[no_mangle]
pub unsafe extern "C" fn codira_struct_type_fields(
    ty: StructInfo,
    fields: *mut Fields,
) -> ErrorHandle {
...
#[no_mangle]
pub unsafe extern "C" fn codira_field_name(field: Field, name: *mut *const c_char) -> ErrorHandle {
    let inner = codira_error_try!(field
        .inner()
        .map_err(|e| format!("invalid argument 'ty': {e}")));
...
/// been deallocated by a previous call to [`codira_type_release`].
#[no_mangle]
pub unsafe extern "C" fn codira_field_type(field: Field, ty: *mut Type) -> ErrorHandle {
    let inner = codira_error_try!(field
        .inner()
        .map_err(|e| format!("invalid argument 'ty': {e}")));
    let store = codira_error_try!(field
        .store()
        .map_err(|e| format!("invalid argument 'ty': {e}")));
...
/// been deallocated by a previous call to [`codira_type_release`].
#[no_mangle]
pub unsafe extern "C" fn codira_field_offset(field: Field, offset: *mut usize) -> ErrorHandle {
    let inner = codira_error_try!(field
        .inner()
        .map_err(|e| format!("invalid argument 'ty': {e}")));
...
mod test {
    ...
    use codira_capi_utils::{
        assert_error_snapshot, assert_getter1, assert_getter3, codira_string_destroy,
    };
    ...
    fn test_codira_struct_type_guid() {
        ...
        assert_getter1!(codira_struct_type_guid(struct_ty, ffi_guid));
    ...
    fn test_codira_struct_type_guid_invalid_null() {
        ...
        assert_error_snapshot!(
            unsafe {
                codira_struct_type_guid(StructInfo(ptr::null(), ptr::null()), guid.as_mut_ptr())
            },
        ...
        assert_error_snapshot!(
            unsafe { codira_struct_type_guid(struct_ty, ptr::null_mut()) },
    ...
    fn test_codira_struct_type_memory_kind() {
        ...
        assert_getter1!(codira_struct_type_memory_kind(struct_ty, memory_kind));
    ...
    fn test_codira_struct_type_memory_kind_invalid_null() {
        ...
        assert_error_snapshot!(
            unsafe { codira_struct_type_memory_kind(struct_ty, ptr::null_mut()) },
    ...
    fn test_codira_struct_type_fields() {
        ...
        assert_getter1!(codira_struct_type_fields(struct_with_fields_struct, fields));
        assert_eq!(fields.count, 2);

        let fields_slice = unsafe { slice::from_raw_parts(fields.fields, fields.count) };

        assert_getter1!(codira_field_name(fields_slice[0], field_name));
        assert_getter1!(codira_field_offset(fields_slice[0], field_offset));
        assert_getter1!(codira_field_type(fields_slice[0], field_type));
        ...
        unsafe { codira_string_destroy(field_name) };
        assert!(unsafe { codira_fields_destroy(fields) }.is_ok());
        assert!(unsafe { codira_type_release(struct_with_fields) }.is_ok());
    }
    ...
    fn test_codira_struct_type_fields_empty() {
        ...
        assert_getter1!(codira_struct_type_fields(empty_struct_struct, fields));
        assert_eq!(fields.count, 0);
        assert!(fields.fields.is_null());
        assert!(unsafe { codira_fields_destroy(fields) }.is_ok());

        assert!(unsafe { codira_type_release(empty_struct) }.is_ok());
    }
    ...
    fn test_codira_fields_find_by_name() {
        ...
        assert_getter1!(codira_struct_type_fields(foo_struct, fields));

        let hello_str = CString::new("hello").unwrap();
        let world_str = CString::new("world").unwrap();

        assert_getter3!(codira_fields_find_by_name(
            fields,
    ...
    fn test_codira_fields_find_by_name_invalid() {
        ...
        assert_error_snapshot!(unsafe {
            codira_fields_find_by_name(Fields { fields: ptr::null(), count: 10}, name.as_c_str().as_ptr(), name.as_bytes().len(), has_field.as_mut_ptr(), found_field.as_mut_ptr())
        }, @r###""invalid argument \'fields\': invalid pointer""###);
        assert_error_snapshot!(unsafe {
            codira_fields_find_by_name(fields, ptr::null(), 0, has_field.as_mut_ptr(), found_field.as_mut_ptr())
        }, @r###""invalid argument \'name\': null pointer""###);
        assert_error_snapshot!(unsafe {
            codira_fields_find_by_name(fields, name.as_c_str().as_ptr(), name.as_bytes().len(), has_field.as_mut_ptr(), ptr::null_mut())
    ...
    fn test_codira_type_name_offset_type_invalid_null() {
        ...
        assert_error_snapshot!(unsafe {
            codira_field_name(Field(ptr::null(), ptr::null()), name.as_mut_ptr())
        }, @r###""invalid argument \'ty\': null pointer""###);
        assert_error_snapshot!(unsafe {
            codira_field_offset(Field(ptr::null(), ptr::null()), offset.as_mut_ptr())
        }, @r###""invalid argument \'ty\': null pointer""###);
        assert_error_snapshot!(unsafe {
            codira_field_type(Field(ptr::null(), ptr::null()), field_type.as_mut_ptr())
        }, @r###""invalid argument \'ty\': null pointer""###);
        ...
        let ty = StructTypeBuilder::new("Foo")
            ...
            .into();
        let (ty, struct_ty) = unsafe { struct_type(ty) };
        assert_getter1!(codira_struct_type_fields(struct_ty, fields));
        assert!(!fields.fields.is_null());

        assert_error_snapshot!(
            unsafe { codira_field_name(*fields.fields, ptr::null_mut()) },
            @r###""invalid argument \'name\': null pointer""###
        );
        assert_error_snapshot!(
            unsafe { codira_field_offset(*fields.fields, ptr::null_mut()) },
            @r###""invalid argument \'offset\': null pointer""###
        );
        assert_error_snapshot!(
            unsafe { codira_field_type(*fields.fields, ptr::null_mut()) },
            @r###""invalid argument \'ty\': null pointer""###
        );
