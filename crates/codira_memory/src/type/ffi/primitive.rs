...
#[no_mangle]
pub extern "C" fn codira_type_primitive(primitive_type: PrimitiveType) -> Type {
...
#[cfg(test)]
mod test {
    use codira_capi_utils::assert_getter1;

    use super::{
        super::{codira_type_kind, TypeKind},
        codira_type_primitive,
        PrimitiveType::{self, *},
    };
    ...
    fn test_primitives() {
        fn test_primitive<T: HasStaticType>(primitive_type: PrimitiveType) {
            let ffi_ty = codira_type_primitive(primitive_type);

            assert_getter1!(codira_type_kind(ffi_ty, ffi_kind));
