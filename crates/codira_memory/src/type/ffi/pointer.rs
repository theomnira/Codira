...
#[no_mangle]
pub unsafe extern "C" fn codira_pointer_type_pointee(
    ty: PointerInfo,
    pointee: *mut Type,
) -> ErrorHandle {
...
/// has been deallocated by a previous call to [`codira_type_release`].
#[no_mangle]
pub unsafe extern "C" fn codira_pointer_is_mutable(
    ty: PointerInfo,
    mutable: *mut bool,
) -> ErrorHandle {
    let ty = codira_error_try!(ty
...
mod test {
    ...
    use codira_capi_utils::{assert_error_snapshot, assert_getter1, assert_getter2};

    use super::super::{
        codira_type_equal, codira_type_kind, codira_type_pointer_type, codira_type_release,
        pointer::{codira_pointer_is_mutable, codira_pointer_type_pointee, PointerInfo},
    ...
    unsafe fn pointer_type(ty: Type, mutable: bool) -> (Type, PointerInfo) {
        assert_getter2!(codira_type_pointer_type(ty, mutable, ptr_ty));

        assert_getter1!(codira_type_kind(ptr_ty, ty_kind));
    ...
    fn test_codira_pointer_type_pointee() {
        ...
        assert_getter1!(codira_pointer_type_pointee(ptr_info, pointee_ty));
        assert!(unsafe { codira_type_equal(pointee_ty, ffi_f32) });

        unsafe { codira_type_release(pointee_ty) };
    ...
    fn test_codira_pointer_type_pointee_invalid_null() {
        ...
        let (ffi_f32_ptr, ptr_info) = unsafe { pointer_type(ffi_f32, true) };
        assert_error_snapshot!(
            unsafe { codira_pointer_type_pointee(ptr_info, ptr::null_mut()) },
    ...
    fn test_codira_pointer_type_is_mutable() {
        ...
        assert_getter1!(codira_pointer_is_mutable(immutable_ptr_info, is_mutable));
        assert!(!is_mutable);

        assert_getter1!(codira_pointer_is_mutable(mutable_ptr_info, is_mutable));
        assert!(is_mutable);

        unsafe { codira_type_release(ffi_f32_mutable_ptr) };
        unsafe { codira_type_release(ffi_f32_immutable_ptr) };
        unsafe { codira_type_release(ffi_f32) };
    }
    ...
    fn test_codira_pointer_type_is_mutable_invalid_null() {
        ...
        let (ffi_f32_ptr, ptr_info) = unsafe { pointer_type(ffi_f32, true) };
        assert_error_snapshot!(
            unsafe { codira_pointer_is_mutable(ptr_info, ptr::null_mut()) },
