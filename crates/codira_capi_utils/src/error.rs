...
#[no_mangle]
pub unsafe extern "C" fn codira_error_destroy(error: ErrorHandle) {
    if !error.0.is_null() {
...
macro_rules! assert_error {
    ($expr:expr $(,)?) => {
        let err = $expr;
        assert!(err.is_err());
        unsafe { $crate::codira_string_destroy(err.0) };
    };
}
    ...
    ($expr:expr, @$snapshot:literal) => {
        ...
        let expr = match unsafe {error_handle.err() } {
            Some(err) => {
                ...
                unsafe { $crate::codira_string_destroy(error_handle.0) };
                err_str
            },
