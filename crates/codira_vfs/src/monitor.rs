...
pub struct MonitorDirectories {
    /// File extensions to monitor (e.g. "code")
...
mod tests {
    ...
    fn test_config() {
        ...
        assert!(!config.contains_file(abs_manifest_dir.join("mod.code")));
        assert!(config.contains_file(abs_manifest_dir.join("src/mod.code")));
        assert!(!config.contains_file(abs_manifest_dir.join("src/mod.rs")));
        assert!(!config.contains_file(abs_manifest_dir.join(".git/src/mod.code")));
        assert!(!config.contains_file(abs_manifest_dir.join("src/.git/mod.code")));
        assert!(config.contains_file(abs_manifest_dir.join("src/.git/special_case/mod.code")));
