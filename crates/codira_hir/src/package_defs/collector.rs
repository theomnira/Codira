...
impl DefCollector<'_> {
    ...
    fn resolve_import(
        ...
        // `resolve_path_with_fixedpoint` only performs name lookups; it does not
        // check whether the resolved item is actually visible from the
        // importing module. Without this filter, a *named* (non-glob) import
        // could reach a `private` item from anywhere, silently bypassing the
        // visibility system entirely (glob imports already filter this way,
        // see the `import.is_glob` branch of `record_resolved_import` below).
        let def = res.resolved_def.and_then(|(item, vis)| {
            if vis.is_visible_from_module_tree(&self.package_defs.module_tree, module_id) {
                Some((item, vis))
            } else {
                None
            }
