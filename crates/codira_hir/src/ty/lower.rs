...
impl Ty {
    ...
    fn from_hir_with_diagnostics(
        ...
        let res = match &type_ref_map[type_ref] {
            ...
            // TODO: `Type?` and `Type { x | pred }` both lower to the type of `Type`
            // itself for now; there is no dedicated `TyKind::Optional` yet, so
            // nil-safety and refinement predicates are not enforced by inference.
