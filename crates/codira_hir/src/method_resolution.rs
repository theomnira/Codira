...
use codira_syntax::{AstNode, AstPtr, SyntaxNodePtr};
...
mod tests {
    ...
    fn test_invalid_self_ty() {
        insta::assert_snapshot!(impl_diagnostics(r#"
            ...
            "#),
            @"
        12..25: inherent `extend` blocks can only be added for structs
        26..41: inherent `extend` blocks can only be added for structs
        ");
    }
    ...
    fn test_duplicate() {
        insta::assert_snapshot!(impl_diagnostics(r#"
            ...
            "#),
            @"
        40..56: the name `bar` is defined multiple times
        71..87: the name `bar` is defined multiple times
        ");
    }
