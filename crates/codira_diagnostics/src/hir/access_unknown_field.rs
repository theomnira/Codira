...
/// #}
/// ```
pub struct AccessUnknownField<'db, 'diag, DB: codira_hir::HirDatabase> {
    db: &'db DB,
    diag: &'diag codira_hir::diagnostics::AccessUnknownField,
    location: TextRange,
}

impl<DB: codira_hir::HirDatabase> Diagnostic for AccessUnknownField<'_, '_, DB> {
...
impl<'db, 'diag, DB: codira_hir::HirDatabase> AccessUnknownField<'db, 'diag, DB> {
    /// Constructs a new instance of `AccessUnknownField`
    pub fn new(db: &'db DB, diag: &'diag codira_hir::diagnostics::AccessUnknownField) -> Self {
