...
impl Function {
    ...
    /// [`codira_runtime::FunctionDefinition`].
    pub unsafe fn inner(&self) -> Result<&codira_runtime::FunctionDefinition, &'static str> {
        self.0
            .cast::<codira_runtime::FunctionDefinition>()
            .as_ref()
            .ok_or("null pointer")
    }
}

impl From<Arc<codira_runtime::FunctionDefinition>> for Function {
    fn from(def: Arc<codira_runtime::FunctionDefinition>) -> Self {
...
#[no_mangle]
pub unsafe extern "C" fn codira_function_add_reference(function: Function) -> ErrorHandle {
...
#[no_mangle]
pub unsafe extern "C" fn codira_function_release(function: Function) -> ErrorHandle {
    if function.0.is_null() {
        return ErrorHandle::new("invalid argument 'function': null pointer");
    }
...
/// a previous call to [`codira_function_release`].
#[no_mangle]
pub unsafe extern "C" fn codira_function_fn_ptr(
    function: Function,
    ptr: *mut *const c_void,
) -> ErrorHandle {
    let function = codira_error_try!(function
        .inner()
...
/// a previous call to [`codira_function_release`].
#[no_mangle]
pub unsafe extern "C" fn codira_function_name(
    function: Function,
...
/// a previous call to [`codira_function_release`].
#[no_mangle]
pub unsafe extern "C" fn codira_function_argument_types(
    function: Function,
    arg_types: *mut Types,
) -> ErrorHandle {
    let function = codira_error_try!(function
...
/// a previous call to [`codira_function_release`].
#[no_mangle]
pub unsafe extern "C" fn codira_function_return_type(
    function: Function,
    ty: *mut Type,
) -> ErrorHandle {
    let function = codira_error_try!(function
...
pub(crate) mod tests {
    ...
    use codira_capi_utils::{
        assert_error_snapshot, assert_getter1, codira_string_destroy, try_convert_c_string,
    };
    use codira_memory::{
        ffi::{codira_type_equal, codira_types_destroy},
        HasStaticType,
    };
    ...
    fn test_function_release_invalid_fn_info() {
        assert_error_snapshot!(
            unsafe { codira_function_release(Function::null()) },
    ...
    fn test_function_add_reference_invalid_fn_info() {
        assert_error_snapshot!(
            unsafe { codira_function_add_reference(Function::null()) },
            @r#""invalid argument \'function\': null pointer""#);
    }

    #[test]
    fn test_function_release_strong_count() {
        let fn_def = codira_runtime::FunctionDefinition::builder("foo").finish();
        let ffi_function: Function = fn_def.clone().into();

        let strong_count = Arc::strong_count(&fn_def);
        assert!(strong_count == 2);

        assert!(unsafe { codira_function_release(ffi_function) }.is_ok());
    ...
    fn test_function_add_reference_strong_count() {
        let function: Function = codira_runtime::FunctionDefinition::builder("foo")
            .finish()
            .into();

        let fn_info_arc = ManuallyDrop::new(unsafe {
            Arc::from_raw(function.0.cast::<codira_runtime::FunctionDefinition>())
        });
        let strong_count = Arc::strong_count(&fn_info_arc);
        assert!(strong_count > 0);

        assert!(unsafe { codira_function_add_reference(function) }.is_ok());
    ...
    fn test_function_invalid_fn_info() {
        let function = codira_runtime::FunctionDefinition::builder("foo")
            .finish()
            .into();

        let mut ptr = MaybeUninit::uninit();
        assert_error_snapshot!(
            unsafe { codira_function_fn_ptr(Function::null(), ptr.as_mut_ptr()) },
            @r#""invalid argument \'function\': null pointer""#);
        assert_error_snapshot!(
            unsafe { codira_function_fn_ptr(function, ptr::null_mut()) },
            @r#""invalid argument \'ptr\': null pointer""#);

        assert!(unsafe { codira_function_release(function) }.is_ok());
    }
    ...
    fn test_function_fn_ptr() {
        ...
        assert_getter1!(codira_function_fn_ptr(function, fn_ptr));
    ...
    fn test_function_name_invalid_fn_info() {
        ...
        let mut ptr = MaybeUninit::uninit();
        assert_error_snapshot!(
            unsafe { codira_function_name(Function::null(), ptr.as_mut_ptr()) },
            @r#""invalid argument \'function\': null pointer""#);
        assert_error_snapshot!(
            unsafe { codira_function_name(function, ptr::null_mut()) },
    ...
    fn test_function_argument_types_invalid_fn_info() {
        ...
        assert_error_snapshot!(unsafe {
            codira_function_argument_types(Function::null(), ptr.as_mut_ptr())
        }, @r#""invalid argument \'function\': null pointer""#);
        assert_error_snapshot!(
            unsafe { codira_function_argument_types(function, ptr::null_mut()) },
    ...
    fn test_function_argument_types_none() {
        ...
        assert_getter1!(codira_function_argument_types(function, arg_types));
        assert_eq!(arg_types.types, ptr::null());
        assert_eq!(arg_types.count, 0);

        assert!(unsafe { codira_types_destroy(arg_types) }.is_ok());
        assert!(unsafe { codira_function_release(function) }.is_ok());
    }
    ...
    fn test_function_argument_types_some() {
        ...
        for arg_type in unsafe { slice::from_raw_parts(arg_types.types, arg_types.count) } {
            assert!(unsafe { codira_type_equal(*arg_type, i32::type_info().clone().into()) });
        }
    ...
    fn test_function_return_type_invalid_fn_info() {
        ...
        let mut ptr = MaybeUninit::uninit();
        assert_error_snapshot!(
            unsafe { codira_function_return_type(Function::null(), ptr.as_mut_ptr()) },
            @r#""invalid argument \'function\': null pointer""#);
        assert_error_snapshot!(
            unsafe { codira_function_return_type(function, ptr::null_mut()) },
    ...
    fn test_function_return_type_none() {
        ...
        assert_getter1!(codira_function_return_type(function, return_type));

        assert!(unsafe { codira_type_equal(return_type, <()>::type_info().clone().into()) });
    }
