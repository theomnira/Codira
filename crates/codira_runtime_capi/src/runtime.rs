...
//!
//! Exposes the Codira runtime using the C ABI.
...
impl Runtime {
    ...
    pub(crate) unsafe fn inner(&self) -> Result<&codira_runtime::Runtime, &'static str> {
        self.0
...
/// data, will lead to undefined behavior.
#[no_mangle]
pub unsafe extern "C" fn codira_runtime_create(
    library_path: *const c_char,
    options: RuntimeOptions,
    handle: *mut Runtime,
) -> ErrorHandle {
    let library_path = codira_error_try!(try_convert_c_string(library_path)
    ...
    let user_functions = codira_error_try!(user_functions
        .into_iter()
        .flatten()
        .map(|def| {
    ...
    let runtime_options = codira_runtime::RuntimeOptions {
        library_path: library_path.into(),
        user_functions,
        type_table,
    };

    let runtime = match codira_runtime::Runtime::new(runtime_options) {
        Ok(runtime) => runtime,
...
#[no_mangle]
pub extern "C" fn codira_runtime_destroy(runtime: Runtime) -> ErrorHandle {
...
/// data, will lead to undefined behavior.
#[no_mangle]
pub unsafe extern "C" fn codira_runtime_find_function_definition(
    runtime: Runtime,
    ...
    let name = codira_error_try!(std::str::from_utf8(slice::from_raw_parts(
        fn_name.cast::<u8>(),
        fn_name_len
    ))
...
/// data, will lead to undefined behavior.
#[no_mangle]
pub unsafe extern "C" fn codira_runtime_get_type_info_by_name(
    runtime: Runtime,
    ...
    let runtime = codira_error_try!(runtime
        ...
        .map_err(|e| format!("invalid argument 'runtime': {e}")));
    let type_name =
        codira_error_try!(try_convert_c_string(type_name)
...
/// data, will lead to undefined behavior.
#[no_mangle]
pub unsafe extern "C" fn codira_runtime_get_type_info_by_id(
    runtime: Runtime,
...
mod tests {
    ...
    use codira_capi_utils::{
        ...
        error::codira_error_destroy,
    };
    use codira_memory::HasStaticType;
    ...
    fn test_runtime_create_invalid_lib_path() {
        assert_error_snapshot!(
            unsafe { codira_runtime_create(ptr::null(), RuntimeOptions::default(), ptr::null_mut()) },
    ...
    fn test_runtime_create_invalid_functions() {
        ...
        assert_error_snapshot!(
            unsafe { codira_runtime_create(lib_path.into_raw(), options, handle.as_mut_ptr()) },
    ...
    fn test_runtime_create_invalid_handle() {
        ...
        assert_error_snapshot!(
            unsafe {
                codira_runtime_create(lib_path.into_raw(), RuntimeOptions::default(), ptr::null_mut())
            },
    ...
    "#,
        ...
        assert_error_snapshot!(
            unsafe {
                codira_runtime_find_function_definition(
                    driver.runtime,
                    ptr::null(),
                    0,
                    ptr::null_mut(),
    ...
    "#,
        ...
        assert_getter3!(codira_runtime_find_function_definition(
            driver.runtime,
            fn_name.as_ptr(),
            fn_name.as_bytes().len(),
            has_fn_info,
    ...
    "#,
        ...
        assert_error_snapshot!(
            unsafe {
                codira_runtime_get_type_info_by_name(
                    driver.runtime,
                    ptr::null(),
                    ptr::null_mut(),
                    ptr::null_mut(),
    ...
    "#,
        ...
        assert_error_snapshot!(
            unsafe {
                codira_runtime_get_type_info_by_id(
                    driver.runtime,
                    ptr::null(),
                    ptr::null_mut(),
                    ptr::null_mut(),
    ...
    "#,
        ...
        let type_id = abi::TypeId::Concrete(abi::Guid([0; 16]));
        assert_getter2!(codira_runtime_get_type_info_by_id(
            driver.runtime,
            &type_id as *const abi::TypeId<'_>,
            has_type_info,
            _type_info,
    ...
    "#,
        ...
        assert_error_snapshot!(
            unsafe { codira_runtime_update(driver.runtime, ptr::null_mut()) },
