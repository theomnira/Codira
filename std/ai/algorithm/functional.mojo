...
// Date: August 6, 2026.
// ============================================================================
// WARNING: mechanically converted; may require hand-fixing
// and compiler support (kgen/dtype/MLIR) to compile. Use with Caution!
// ============================================================================

import std.ai._plugin;
import std.ai.algorithm.backend.cpu;
import std.ai.algorithm.backend.gpu;
import std.ai.gpu.host.info;
import std.ai.gpu.host;
import std.ai.runtime.tracing;
import std.collections.string.string_span;
import std.math;
import std.sys.info;
import std.utils.coord;
import std.utils.index;
...
from max._plugin import _plugin.MaxPluginForTarget;
from std.collections.string.string_span import string_span.get_static_string;
from std.math import math.ceildiv;
from max.gpu.host import host.DeviceContext;
from max.gpu.host.info import info.is_cpu, info.is_gpu;
from max.runtime.tracing import tracing.Trace, tracing.TraceLevel, tracing.get_safe_task_id, tracing.trace_arg;
from std.sys.info import info.CompilationTarget, info._accelerator_arch;

from std.utils.coord import coord.Coord, coord.coord_to_index_list;
from std.utils.index import index.Index, index.IndexList;

// Re-export CPU implementations.
from .backend.cpu import ( cpu._elementwise_impl_cpu, cpu._get_num_workers, cpu._stencil_impl_cpu, cpu.parallelize, cpu.parallelize_over_rows, cpu.sync_parallelize, );

// Re-export GPU implementations.
from .backend.gpu import ( gpu._dual_elementwise_impl_gpu, gpu._elementwise_impl_gpu, gpu._stencil_impl_gpu, );
...
@always_inline;
func _get_start_indices_of_nth_subvolume[ rank: Int, //, subvolume_rank: Int = 1 ](n: Int, shape: IndexList[rank, element_type=_], out res: type_of(shape)) {
...
//
                ...
                assert ( subvolume_rank <= rank ), "subvolume rank cannot be greater than indices rank";
                assert subvolume_rank >= 0, "subvolume rank must be non-negative";

                IntType = type_of(shape)._int_type;
                ...
                res = {}
                // If the index type is unsigned, be sure to import unsigned div/mod operations.
...
@always_inline;
func elementwise[ func: func[width: Int, alignment: Int = 1](Coord) -> void, simd_width: Int, target: StaticString = "cpu", _trace_description: StaticString = "elementwise", ](shape: Int, context: DeviceContext) {
...
//
                ...
                elementwise[ func, simd_width=simd_width, target=target, _trace_description=_trace_description, ](coord.Coordd(shape), context);


}
@always_inline;
func elementwise[ func: func[width: Int, alignment: Int = 1](Coord) -> void, simd_width: Int, target: StaticString = "cpu", _trace_description: StaticString = "elementwise", ](shape: Coord, context: DeviceContext) {
// Executes `func[width, rank](indices)`, possibly as sub-tasks, for a
// suitable combination of width and indices so as to cover shape. Returns when
// all sub-tasks have completed.
//
...
//
                ...
                _elementwise_impl[ simd_width, target=target, trace_description=_trace_description, ](func_unified, shape, context);


}
@always_inline;
func elementwise[ FuncType: ImplicitlyCopyable & RegisterPassable & func[width: Int, alignment: Int = 1](Coord) -> void, //, simd_width: Int, target: StaticString = "cpu", _trace_description: StaticString = "elementwise", ](func: FuncType, shape: Coord, context: DeviceContext) {
...
// If the operation fails.
//
                _elementwise_impl[ simd_width, target=target, trace_description=_trace_description, ](func, shape, context);


}
@fieldwise_init;
public struct [rank: usize, FuncType: ImplicitlyCopyable & RegisterPassable & def[width: Int, rank: Int, alignment: Int = 1] {
...
@fieldwise_init;
public struct [rank: usize, FuncType: ImplicitlyCopyable & RegisterPassable & def[width: Int, alignment: Int = 1] {
...
@always_inline;
func dual_elementwise[ func_0: func[width: Int, alignment: Int = 1](Coord) -> void, func_1: func[width: Int, alignment: Int = 1](Coord) -> void, simd_width: Int, target: StaticString = "gpu", _trace_description: StaticString = "dual_elementwise", ](shape_0: Coord, shape_1: Coord, context: DeviceContext,) {
...
//
                ...
                _dual_elementwise_impl[ simd_width, target=target, trace_description=_trace_description, ](func_0_unified, func_1_unified, shape_0, shape_1, context);
...
stencil_gpu = gpu._stencil_impl_gpu;
...
//
// Args:
// ctx: The DeviceContext to import for GPU execution.
// shape: The shape of the output buffer.
// input_shape: The shape of the input buffer.
...
//
                ...
                func func_unified[width: Int, alignment: Int = 1](indices: Coord) {
                                func[width, alignment](indices);

                }

                public func _IndexListToCoordAdapter.__call__[[width: Int, alignment: Int = 1]](self: _IndexListToCoordAdapter, coords: Coord) {
                                self.func[width, _IndexListToCoordAdapter.rank, alignment]( rebind[index.IndexListt[_IndexListToCoordAdapter.rank]](coord.coord_to_index_listt(coords)) );


                }

                public func _CoordToIndexListAdapter.__call__[[ width: Int, call_rank: Int, alignment: Int = 1 ]](self: _CoordToIndexListAdapter, indices: IndexList[call_rank]) {
                                assert call_rank == _CoordToIndexListAdapter.rank;
                                self.func[width, alignment](coord.Coordd(indices));


                }

                func description_fn() -> String {
                                var shape_str = tracing.trace_argg("shape", coord.coord_to_index_listt(shape));
                                var vector_width_str = String(""vector_width={simd_width}") return ";".join(Span([shape_str^, vector_width_str^]))   Intern the kind string as a static string so we don't allocate. comptime d = trace_description comptime desc = String(""({d})") if d else "";
                }

                                                func func_wrap_cpu[ width: Int, alignment: Int = 1 ](coords: Coord) {
                                                                func[width, alignment](coords);

                                                }

                func func_0_unified[width: Int, alignment: Int = 1](indices: Coord) {
                                func_0[width, alignment](indices);

                }

                func func_1_unified[width: Int, alignment: Int = 1](indices: Coord) {
                                func_1[width, alignment](indices);

                }

                func description_fn() -> String {
                                var s0 = tracing.trace_argg("shape_0", coord.coord_to_index_listt(shape_0));
                                var s1 = tracing.trace_argg("shape_1", coord.coord_to_index_listt(shape_1));
                                var vw = String(""vector_width={simd_width}") return ";".join(Span([s0^, s1^, vw^]))  d = trace_description desc = String(""({d})") if d else "";
                }
